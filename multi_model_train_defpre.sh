#!/bin/bash
num_of_model=$(ls -1 $1/*ref.pth | wc -l)
sequence=($(seq 1 $num_of_model))
list_of_ref=($(ls $1/*ref.pth))
for i in "${!list_of_ref[@]}"; do
  #bd_t=$(cat imagenette_candidate_permut.out | head -n ${sequence[i]} | tail -n 1)
  bd_t=$(cat cifar10_candidate_permut.out | head -n ${sequence[i]} | tail -n 1)
