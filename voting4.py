...
train_size = int(sys.argv[1]) # per class; 0 for full set (must be balanced), -1 for cross-validation
...
thresholding = int(sys.argv[7]) # 0: constant, 1: max J, 2: weighted mean of means, 3: normal fitting
...
for experiment in range(len(all_models) if crossval else train_count):
	if crossval:
		test_model = all_models[experiment]
	...
	if crossval:
		Plist.append((get_prediction(get_score(test_model,train_clean_set,train_backdoor_set)),backdoor(test_model)))
	else:
